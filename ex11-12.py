...
#Этот код определяет функцию filter_numbers, которая принимает переменное количество аргументов *args. Затем она проходит по каждому числу и проверяет, принадлежит ли оно интервалу [1, 3]. Если число удовлетворяет условию, оно добавляется в список filtered_numbers. В конце функция возвращает этот список.
